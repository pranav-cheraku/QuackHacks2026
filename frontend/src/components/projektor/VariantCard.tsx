...
export function VariantCard({ v }: { v: Variant }) {
  return (
    ...
    >
      <div
        className={`rounded-md border bg-card shadow-sm overflow-hidden ${v.chosen ? "border-[color:var(--accent-teal)] ring-1 ring-[color:var(--accent-teal)]/30" : "border-border opacity-70"}`}
      >
        <div className="h-[90px] bg-white p-2 relative">
          {v.layout === "stacked" && (
            <div className="space-y-1.5">
              ...
              <div
                className="h-6 w-full mt-1 rounded-sm"
      ...
      <div className="mt-1 text-center text-[9px] font-mono">
        {v.chosen ? (
          <span
            className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-white"
