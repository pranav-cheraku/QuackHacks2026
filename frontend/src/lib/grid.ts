...
export function pixelRectToPlacement(
  ...
  const tl = pixelsToGrid(
    Math.min(x1, x2),
    Math.min(y1, y2),
    slideWidth,
    slideHeight,
  );
  const br = pixelsToGrid(
    Math.max(x1, x2),
    Math.max(y1, y2),
    slideWidth,
    slideHeight,
...
export function validatePlacement(p: GridPlacement): ValidationResult {
  ...
  if (!Number.isFinite(p.col) || p.col < 0)
    errors.push(`col must be >= 0 (got ${p.col})`);
  if (!Number.isFinite(p.row) || p.row < 0)
    errors.push(`row must be >= 0 (got ${p.row})`);
  if (!Number.isFinite(p.colSpan) || p.colSpan < 1)
    errors.push(`colSpan must be >= 1 (got ${p.colSpan})`);
  if (!Number.isFinite(p.rowSpan) || p.rowSpan < 1)
    errors.push(`rowSpan must be >= 1 (got ${p.rowSpan})`);
  if (p.col + p.colSpan > GRID_COLS)
    errors.push(
      `col(${p.col}) + colSpan(${p.colSpan}) = ${p.col + p.colSpan} exceeds GRID_COLS(${GRID_COLS})`,
    );
  if (p.row + p.rowSpan > GRID_ROWS)
    errors.push(
      `row(${p.row}) + rowSpan(${p.rowSpan}) = ${p.row + p.rowSpan} exceeds GRID_ROWS(${GRID_ROWS})`,
    );
  ...
  const colSpan = Math.max(
    1,
    Math.min(GRID_COLS - col, Math.round(p.colSpan ?? SNAP_STEP)),
  );
  const rowSpan = Math.max(
    1,
    Math.min(GRID_ROWS - row, Math.round(p.rowSpan ?? SNAP_STEP)),
  );
