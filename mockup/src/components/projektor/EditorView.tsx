...
import {
  ...
  Square, Layers, Trash2, ChevronDown,
} from "lucide-react";
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-chrome">
      ...
      <div className="h-10 border-b border-border bg-chrome flex items-center px-3 gap-1 text-ink shrink-0 text-[12px]">
        <ToolIcon><Undo2 size={13} /></ToolIcon>
        <ToolIcon><Redo2 size={13} /></ToolIcon>
        <Sep />
        <Dropdown label="100%" />
        ...
        <Sep />
        <ToolIcon><Bold size={12} /></ToolIcon>
        <ToolIcon><Italic size={12} /></ToolIcon>
        <ToolIcon><Underline size={12} /></ToolIcon>
        <ToolIcon><Strikethrough size={12} /></ToolIcon>
        <div className="w-5 h-5 rounded border border-border mx-1" style={{ background: "var(--accent-teal)" }} />
        <Sep />
        <ToolIcon><AlignLeft size={12} /></ToolIcon>
        <ToolIcon><AlignCenter size={12} /></ToolIcon>
        <ToolIcon><AlignRight size={12} /></ToolIcon>
        <Sep />
        <ToolIcon><Type size={12} /></ToolIcon>
        <ToolIcon><ImageIcon size={12} /></ToolIcon>
        <ToolIcon><Square size={12} /></ToolIcon>
        <Sep />
        <ToolIcon><Layers size={12} /></ToolIcon>
        <ToolIcon><Trash2 size={12} /></ToolIcon>
        <div className="ml-auto flex items-center gap-2 text-[10px] font-mono text-muted-foreground">
          {["X","Y","W","H"].map(k => (
            <div key={k} className="flex items-center gap-1">
              <span>{k}</span>
              <input defaultValue={k === "W" ? "928" : k === "H" ? "522" : "0"} className="w-12 px-1.5 py-0.5 border border-border rounded text-ink bg-card" />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <aside className="w-[224px] shrink-0 border-r border-border bg-chrome flex flex-col">
          ...
          <div className="overflow-y-auto px-3 pb-3 space-y-2">
            {INITIAL_NODES.map(n => (
              ...
              >
                <div className="text-[10px] font-mono text-muted-foreground pt-1 w-5">{String(n.index).padStart(2,"0")}</div>
                <div className={`flex-1 rounded border bg-card overflow-hidden h-[100px] ${active === n.id ? "border-[color:var(--accent-teal)] ring-1 ring-[color:var(--accent-teal)]/30" : "border-border"}`}>
                  {n.state === "rendered" ? <SlideThumb node={n} /> : (
                    <div className="h-full w-full bg-canvas/40 flex items-center justify-center text-[8px] font-mono text-muted-foreground">ingredient</div>
                  )}
                </div>
              </button>
            ))}
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-canvas/60">
          <div className="flex-1 flex items-center justify-center p-8 overflow-auto">
            <div className="bg-white shadow-[0_10px_40px_-15px_oklch(0.3_0.01_175/0.3)] rounded-sm relative overflow-hidden" style={{ width: 928, height: 522 }}>
              ...
              <div className="absolute right-0 top-0 h-full w-12 flex items-center justify-center" style={{ background: "var(--accent-teal)" }}>
                <div className="text-white font-mono text-[10px] tracking-[0.4em] font-bold" style={{ writingMode: "vertical-rl", transform: "rotate(180deg)" }}>
                  PROJEKTOR
                </div>
              </div>
              <div className="p-12 h-full flex flex-col">
                <div className="text-[12px] font-mono uppercase tracking-[0.25em] font-bold" style={{ color: "var(--accent-teal)" }}>
                  Q3 · FY26
                </div>
                <div className="mt-10 text-[72px] leading-[0.95] font-extrabold text-ink tracking-tight" style={{ fontWeight: 800 }}>
                ...
                <div className="mt-auto flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-[12px] font-bold" style={{ background: "oklch(0.55 0.1 250)" }}>AK</div>
                  <div>
          ...
          <div className="h-[100px] border-t border-border bg-chrome px-4 py-2 shrink-0">
            <div className="text-[9px] font-mono uppercase tracking-wider text-muted-foreground">Speaker notes</div>
            <textarea
        ...
        <aside className="w-[220px] shrink-0 border-l border-border bg-chrome flex flex-col">
          <div className="flex border-b border-border text-[11px] font-semibold">
            {(["format","transition","layout"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2 capitalize transition-colors ${tab === t ? "text-ink border-b-2 border-[color:var(--accent-teal)] -mb-px" : "text-muted-foreground hover:text-ink"}`}>
                {t}
              </button>
            ))}
          </div>
          <div className="p-3 space-y-3 text-[11px] overflow-y-auto">
            ...
            <PanelRow label="Color">
              <div className="flex items-center gap-1.5">
                <div className="w-3.5 h-3.5 rounded border border-border" style={{ background: "var(--ink)" }} />
                <span className="font-mono text-[10px]">#1F2422</span>
              </div>
            </PanelRow>
            <div className="pt-2 border-t border-border">
              <div className="text-[9px] font-mono uppercase tracking-wider text-muted-foreground mb-2">Fill & border</div>
            ...
            <div className="pt-2 border-t border-border">
              <div className="text-[9px] font-mono uppercase tracking-wider text-muted-foreground mb-2">Swatches</div>
              <div className="grid grid-cols-6 gap-1.5">
                {["oklch(0.24 0.009 185)","var(--accent-teal)","oklch(0.55 0.1 250)","oklch(0.65 0.12 30)","oklch(0.6 0.11 140)","oklch(0.99 0 0)"].map(c => (
                  <div key={c} className="w-6 h-6 rounded border border-border" style={{ background: c }} />
                ))}
              </div>
            </div>
          </div>
...
function ToolIcon({ children }: { children: React.ReactNode }) {
  return <button className="w-7 h-7 flex items-center justify-center rounded hover:bg-canvas/60 text-ink">{children}</button>;
}
