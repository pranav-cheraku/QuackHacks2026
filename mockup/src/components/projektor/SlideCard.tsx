...
function chipStyle(c: ComponentType): string {
  ...
  if (c === "Stat") return "border-[color:var(--accent-teal)]/40 bg-[color:var(--accent-soft)]/40 text-ink";
  if (["Image","Chart","Video","Divider"].includes(c)) return "bg-canvas/60 border-border text-ink";
  return "bg-white border-border text-muted-foreground";
}

export function SlideCard({ node, selected, onSelect, onOpenEditor, onMove, onDropComponent, zoom }: Props) {
  ...
  const onMouseDown = (e: React.MouseEvent) => {
    ...
    const move = (ev: MouseEvent) => {
      ...
      onMove(startX + (ev.clientX - dragging.current.ox) / zoom, startY + (ev.clientY - dragging.current.oy) / zoom);
    };
  ...
  return (
    <div
      ...
      style={{ left: node.x, top: node.y, width: w, transform: `rotate(${node.rotation}deg)`, transformOrigin: "center" }}
    ...
    >
      ...
      >
        {node.state === "rendered" ? (
          <div style={{ height: h }}><SlideThumb node={node} /></div>
        ) : (
          <div className="p-2.5 flex flex-col gap-1.5">
            {node.components.length === 0 ? (
              <div className="text-[10px] text-muted-foreground text-center py-8 font-mono">Drop components here</div>
            ) : node.components.map((c, i) => (
              <div key={i} className={`px-2 py-1 text-[10px] rounded-[4px] border ${chipStyle(c)}`}>
                {c}
              </div>
            ))}
            <button
        ...
        {node.state === "rendered" && (
          <div className="absolute top-1.5 right-1.5 flex items-center gap-0.5 text-[8px] font-mono px-1.5 py-0.5 rounded-full bg-white/90 border border-border" style={{ color: "var(--accent-teal)" }}>
